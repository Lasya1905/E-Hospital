    ...
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
        ...
        <div className='w-full grid grid-cols-[var(--grid-template-columns-auto)] gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
            {relDoc.slice(0,5).map((doctor, index) => (
                <div className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={index} onClick={()=>{navigate(`/appointment/${doctor._id}`); scrollTo(0,0)}}>
